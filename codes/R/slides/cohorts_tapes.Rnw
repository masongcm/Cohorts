...
\bi
  ...
  \item Most economic literature uses simple sum scores of these scales
  \item Sometimes useful to consider different populations/groups
    \bi
    \item Different countries
    \item Different subgroups (gender, race, ...)
    \item Same population, at different points in time
    \ei
  \item Do these scales measure the same constructs?
  \item Are the constructs measured in the same way?
...
We compare the evolution of socioemotional skills in the UK
\bi
\item \textbf{Q1}: \textbf{How many latent dimensions} do we need to capture socioemotional skills in the data?
~\\
\pause
\item \textbf{Q2}: Can we derive \textbf{comparable measures} of socioemotional skills across cohorts of children born in different years?
~\\
\pause
\item \textbf{Q3}: How has \textbf{inequality} in early-life socioemotional skills evolved in the UK between 1975 and 2005?
~\\
\pause
\item \textbf{Q4}: Have the \textbf{determinants} of socioemotional skills evolved?
~\\
\pause
\item \textbf{Q5}: How do early socioemotional skills map into \textbf{adolescent/adult outcomes}?
\ei
\end{frame}
...
\item Rutter A scale (BCS) 
  ...
  \item Traditionally conceptualised as bidimensional \citep{Blanden2007}
    ...
    \ei
  \item Sometimes the former is split into aggressive/conduct vs hyperkinetic \citep{McGee1985,Collishaw2004}
  \ei
...
print(xtable(table,
             ...
             add.to.row = list(pos = c(list(0)),
                              command = c(
                              # header
            " & & \\multicolumn{2}{c}{\\textbf{Full sample}} & \\multicolumn{2}{c}{\\textbf{59-61 months}} \\\\ \\cmidrule(lr){3-4}  \\cmidrule(lr){5-6}
...
\item Is this an improvement on using simple sum scores?
  \bi
  \item Sum scores assume that each item conveys the same information
...
\bi
\item \textbf{Q3}: How has socioeconomic inequality in socioemotional skills increased between 1975 (BCS) and 2005 (MCS)?
\item 
\ei
...
\vspace{-.2cm}
\begin{figure}
<<FACINEQ_SM, dependson="FA_SCORES_BIND", dev='tikz', fig.height=7.5, fig.width=11, out.width='\\linewidth'>>=
# plot
...
\item Inequality across socioeconomic indicators has increased, especially
  ...
  \item considering inequality across maternal schooling and employment, rather than paternal occupation
  \ei
\ei
...
We explain externalising and internalising skills using information from the birth and 5 year surveys \\
...
\framesubtitle{Explaining the gradient by maternal compulsory schooling \hyperlink{gelb_ext}{\beamerbutton{Table}}}

\end{frame}

\begin{frame}
\frametitle{Decomposition (INT)}\label{gelb_int_f}
\framesubtitle{Explaining the gradient by maternal compulsory schooling \hyperlink{gelb_int}{\beamerbutton{Table}}}
...
\section{Mapping skills to later outcomes}
...
\begin{frame}
\frametitle{Mapping skills into behaviours}
...
\begin{frame}
\frametitle{Mapping skills into labour market}
