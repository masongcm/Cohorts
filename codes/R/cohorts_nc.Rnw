...
The model can be used to investigate how inequality of noncognitive skills has changed between the two cohorts. The variances of the $EXT$ and $INT$ factors are estimated to be larger in the MCS children by \Sexpr{round(lv.covs$MCS[1,1]-1,2)} and \Sexpr{round(lv.covs$MCS[2,2]-1,2)} of a standard deviation, respectively, indicating larger dispersion.
...
\begin{figure}
<<FACLOESS_INC, dev='tikz', fig.height=5, fig.width=10, out.width='1\\linewidth'>>=
@
