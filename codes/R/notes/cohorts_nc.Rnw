...
\clearpage
\begin{figure}
<<FACDENS, dependson="FA_SCORES_BIND", dev='tikz', fig.height=10, fig.width=10, out.width='1\\linewidth'>>=
@
...
\clearpage
\begin{figure}
<<RAWHIST, dependson="FA_SCORES_BIND", dev='tikz', fig.height=10, fig.width=10, out.width='1\\linewidth'>>=
@
...
\centering
\small
\caption{Inequality measures for behavioural skills}\label{tab:ineqscores}
...
print(xtable(table,
                    align="L{0cm}L{.20\\textwidth}C{.15\\textwidth}C{.15\\textwidth}C{.15\\textwidth}C{.15\\textwidth}",
                    digits = 4
                    ),
             size="\\small",
             include.rownames=F,
...
\clearpage
\begin{figure}
<<FACINEQ_SC, dependson="FA_SCORES_BIND", dev='tikz', fig.height=12, fig.width=10, out.width='1\\linewidth'>>=
# plot
...
\clearpage
\begin{figure}
<<FACINEQ_YS, dependson="FA_SCORES_BIND", dev='tikz', fig.height=12, fig.width=10, out.width='1\\linewidth'>>=
...
\clearpage
\par\vfill\break % break page to change margins
\advance\hoffset by -1cm % Shift top margin
\begin{landscape}
\begin{figure}
<<FACINEQ_MECS, dependson="FA_SCORES_BIND", dev='tikz', fig.height=10, fig.width=14, out.width='1\\linewidth'>>=
fi <- fi_mecs # select result
gridExtra::grid.arrange(fi$ineqlist[[1]],fi$ineqlist[[2]], 
                        fi$ineqlist[[3]],fi$ineqlist[[4]],
                        fi$histlist[[1]],fi$histlist[[2]], fi$legend,
                        ncol=2, nrow=4, heights = c(4,4,1.5,0.3))
@
\caption{Mean scores across maternal employment and schooling}\label{fig:meanci_me}
{\scriptsize \emph{Notes}: The figure plots the mean and its 95\% confidence interval across interactions between a dummy for whether the cohort member's mother continued education past compulsory schooling age, and employment status of the cohort member's mother at 5 years of age. \par}
\end{figure}
\end{landscape}
\par\vfill\break
\advance\hoffset by 1cm % Shift top margin back to normal


\clearpage
\begin{figure}
<<FACINEQ_SM, dependson="FA_SCORES_BIND", dev='tikz', fig.height=12, fig.width=10, out.width='1\\linewidth'>>=
...
gridExtra::grid.arrange(fi$ineqlist[[1]],fi$ineqlist[[2]], 
                        ...
                        ncol=2, nrow=4, heights = c(4,4,2,0.5))
@
\caption{Mean scores across maternal marital status}\label{fig:meanci_sm}
{\scriptsize \emph{Notes}: The figure plots the mean and its 95\% confidence interval by marital status of the cohort member's mother at 5 years of age. \par}
\end{figure}
...
library(stargazer)
stargazer(r.ext$BCS.M[[1]], r.ext$BCS.M[[2]], r.ext$MCS.M[[1]], r.ext$MCS.M[[2]], r.int$BCS.M[[1]], r.int$BCS.M[[2]], r.int$MCS.M[[1]], r.int$MCS.M[[2]],
          column.labels=c("BCS","BCS","MCS","MCS","BCS","BCS","MCS","MCS"),
...
\\[-12pt] \justify {\scriptsize \emph{Notes}: This table shows coefficients from regressions of externalising and internalising skills on potential determinants.
\par}
\end{table}
\end{landscape}
...
library(stargazer)
stargazer(r.ext$BCS.F[[1]], r.ext$BCS.F[[2]], r.ext$MCS.F[[1]], r.ext$MCS.F[[2]], r.int$BCS.F[[1]], r.int$BCS.F[[2]], r.int$MCS.F[[1]], r.int$MCS.F[[2]],
          column.labels=c("BCS","BCS","MCS","MCS","BCS","BCS","MCS","MCS"),
          dep.var.caption = "", multicolumn = FALSE,
          #omit=c("Constant", paste0("region", c(2:9))),
          no.space=TRUE,
...
\begin{landscape}
\begin{table}[!htp]
\caption{Gelbach decomposition of the EXT/INT differential by maternal education}
\centering
\input{../../../tables/med_gelb}
\\[-12pt] \justify {\scriptsize \emph{Notes}: This table shows the decomposition of the differential in externalising and internalising skills between children whose mothers are educated past the compulsory schooling age, and children whose mothers left school at the minimum school leaving age. Education of mothers is observed when the child is aged around 5 (1975 for BCS, 2005 for MCS). Col. (1) shows: the unconditional difference, i.e. the coefficient on maternal education in a regression that controls only for region of birth; the conditional difference, i.e. the same coefficient after adding explanatory variables; and the portion of the unconditional difference explained by each group of variables. Col. (2) shows the fraction of the total difference due to each group of variables. The decomposition follows Gelbach (2016), and is invariant to the order in which the explanatory variables are added to the conditional model.
\par}
\end{table}
\end{landscape}
\par\vfill\break
...
\begin{table}
\caption{Effect of behavioural skills on adolescent outcomes -- BCS}
\centering
\resizebox{\linewidth}{!}{
<<REGS_OUTC_BCS, results='asis', dependson="REGRESSIONS">>=

# collapse to matrix
bcs_outctab_mat <- do.call(rbind,lapply(bcs_outctab,matrix,ncol=11,byrow=FALSE))
table <- bcs_outctab_mat

print(xtable(table,
                    align="R{0cm}L{.25\\textwidth}C{.06\\textwidth}C{.08\\textwidth}C{.08\\textwidth}C{.08\\textwidth}C{.08\\textwidth}C{.06\\textwidth}C{.08\\textwidth}C{.08\\textwidth}C{.08\\textwidth}C{.08\\textwidth}",
                    digits = 3
                    ),
             size="\\small",
             include.rownames=F,
...
\\[-12pt] \justify {\scriptsize \emph{Notes}: This table shows coefficients from regressions of adolescent and adult outcomes on externalising and internalising skills, also conditioning on early childhood variables.
\par}
\end{table}


\begin{table}
\caption{Effect of behavioural skills on adolescent outcomes -- MCS}
\centering
\resizebox{\linewidth}{!}{
...
# expand dummies
datasum <- cbind(
  finaldata[,c("EXT","INT","cohortsex")], 
  dummy.data.frame(finaldata[,c(decvars)], sep = "")
)
