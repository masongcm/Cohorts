...
table <- meantab[[2]] # females (but just need item names)
...
\framesubtitle{Results \hyperlink{measpars}{\beamerbutton{Measurement}}\hyperlink{lvpars}{\beamerbutton{Latent variable}}}

\bi
\item Restricting thresholds and loadings provides a good fit \hyperlink{fit}{\beamerbutton{Fit}}
...
\vspace{-.2cm}
\begin{figure}
<<FACINEQ_MECS, dependson="FA_SCORES_BIND", dev='tikz', fig.height=8.5, fig.width=13.5, out.width='\\linewidth'>>=
# plot
...
# select models
regs <- list(r_ext$BCS.M[["OLS"]],r_ext$MCS.M[["OLS"]],r_int$BCS.M[["OLS"]],r_int$MCS.M[["OLS"]])
...
texreg::texreg(regs,
               ...
               custom.model.names = c("EXT BCS", "EXT MCS", "INT BCS", "INT MCS"),
               table = FALSE
               )
...
# select models
regs <- list(r_ext$BCS.F[["OLS"]],r_ext$MCS.F[["OLS"]],r_int$BCS.F[["OLS"]],r_int$MCS.F[["OLS"]])
...
\begin{frame}
\frametitle{Decomposition (EXT)}
\framesubtitle{Explaining the education gradient}
\centering
\scriptsize
\resizebox{\textwidth}{!}{
...
# collapse to matrix (first 4 elements of bcs_outctab are adolescence)
bcs_outctab_mat <- do.call(rbind,lapply(bcs_outctab[c(1:4)],matrix,ncol=11,byrow=FALSE))
table <- bcs_outctab_mat

print(xtable(table,
                    align="R{0cm}L{.22\\textwidth}C{.05\\textwidth}C{.075\\textwidth}C{.075\\textwidth}C{.075\\textwidth}C{.075\\textwidth}C{.05\\textwidth}C{.075\\textwidth}C{.075\\textwidth}C{.075\\textwidth}C{.075\\textwidth}",
                    digits = 3
                    ),
             size="\\scriptsize",
             include.rownames=F,
