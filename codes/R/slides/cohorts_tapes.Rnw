...
\date[NCDS]{NCDS 60 Years of Our Lives \\ \today}
...
\bi
\item Non-cognitive (socio-emotional, behavioural) skills developed during childhood are important for lifetime outcomes
\item Socioeconomic gradients in these skills are persistent 
\pause 
\item \textbf{Q1}: Can we derive \textbf{comparable measures} across populations?
    \bi
    ...
    \ei
\pause
\item \textbf{Q2}: Can we use these measures to document \textbf{changes in socioeconomic inequality} in skills across populations?
\ei
...
\vspace{6pt} \pause
\bnm
\item Construct a common 11-item scale of non-cognitive skills, starting from different instruments
\item Investigate its psychometric properties
...
\item \textbf{Key findings}:
  ...
  \item Mostly driven by improvements at the top of the distribution
  \ei
\ei
...
\bi
\item Data from 5 year follow-up (England only), maternal questionnaire
...
\bi
\item Data from 5 year follow-up (England only), maternal questionnaire
  \bi \item Complex survey design: rebalance sample to make representative \ei
...
\item Factors can be coherently interpreted as internalising and internalising skills
\ei 
\vspace{12pt}
\centering
...
\item \textbf{Measurement Invariance (MI)} analysis
  \bi
  \item Formal test of the invariance hypothesis
  \item Set up categorical Item Factor Analysis (IFA) model \hyperlink{miapp1}{\beamerbutton{Det.}}
  \item Pool the two cohorts together, and estimate models of different invariance level -- following \cite{Wu2016}:\hyperlink{miapp2}{\beamerbutton{Det.}}
    \bnm
    \item Start from minimally restrictive model, where parameters are free to vary
    \item Restrict threshold and loading parameters to be equal across cohorts
    \item Restrict intercept parameters as well
    \enm
...
<<FA_SCORES_MERGE, dependson='LOAD_DATA'>>=
...
<<FA_SCORES_SELECT, dependson='FA_SCORES_COMPARE'>>=
...
\bi
\item Restricting thresholds and loadings provides a good fit
...
\item Models with restricted intercepts do \emph{not} pass MI test
  \bi
  \item Unclear whether mean comparisons can be done
  \item \cite{Wu2016} vs \cite{Millsap2004}
  \ei
% \item Might \emph{age differences} in the sample be playing a role?
%   \bi
%   \item BCS and MCS samples differ due to survey design \hyperlink{agehist}{\beamerbutton{Graph}}
%   \item But restricting the age range to maximise overlap does not give significantly different results \hyperlink{fit4}{\beamerbutton{Det.}}
%   \ei
  \pause
\item \textbf{Q1}: Can we derive comparable measures of noncognitive skills across cohorts?
\item Yes, we can use the model with non-invariant intercepts to score the latent factors (externalising and internalising skills)
\ei
...
\begin{frame}
\frametitle{Inequality}\label{meanci}
\framesubtitle{Visualising the gradient \hyperlink{loessext}{\beamerbutton{EXT}}\hyperlink{loessint}{\beamerbutton{INT}}}
\begin{figure}
...
% \centering
% \resizebox{!}{4cm}{%
% <<FA_REGS_REAL, dependson='FA_SCORES_SELECT', results='asis'>>=
...
%                 paste0("Income Q5 [",toString(minmax.real[5,1]),"+ £/w]"),
%                              "MCS dummy",
%                              "Income Q2 x MCS",
%                              "Income Q3 x MCS",
%                              "Income Q4 x MCS",
...
\begin{frame}
\frametitle{Inequality}
\framesubtitle{Quantifying the gradient}
\centering
\resizebox{12cm}{!}{%
<<FA_REGS_REAL_MF, dependson='FA_SCORES', results='asis'>>=
# regress scores on income
library(dplyr)
library(stargazer)


r.ext.m.bcs <- lm(EXT ~ as.factor(incq), data=subset(scores2plot, sex=="M" & cohort=="BCS"))
r.ext.m.mcs <- lm(EXT ~ as.factor(incq), data=subset(scores2plot, sex=="M" & cohort=="MCS"))
r.int.m.bcs <- lm(INT ~ as.factor(incq), data=subset(scores2plot, sex=="M" & cohort=="BCS"))
r.int.m.mcs <- lm(INT ~ as.factor(incq), data=subset(scores2plot, sex=="M" & cohort=="MCS"))

r.ext.f.bcs <- lm(EXT ~ as.factor(incq), data=subset(scores2plot, sex=="F" & cohort=="BCS"))
r.ext.f.mcs <- lm(EXT ~ as.factor(incq), data=subset(scores2plot, sex=="F" & cohort=="MCS"))
r.int.f.bcs <- lm(INT ~ as.factor(incq), data=subset(scores2plot, sex=="F" & cohort=="BCS"))
r.int.f.mcs <- lm(INT ~ as.factor(incq), data=subset(scores2plot, sex=="F" & cohort=="MCS"))
...
% \begin{frame}
% \frametitle{Inequality}
% \framesubtitle{Quantifying the gradient -- males}
% \centering
% \resizebox{12cm}{4cm}{%
% <<FA_REGS_REAL_M, dependson='FA_SCORES', results='asis'>>=
% # regress scores on income
% library(dplyr)
% library(stargazer)
% 
% 
% r.ext.m.bcs <- lm(EXT ~ as.factor(incq), data=subset(scores2plot, sex=="M" & cohort=="BCS"))
% r.ext2.m.bcs <- lm(EXT ~ as.factor(incq) + bwt + as.factor(smkpr) + ysch_fath + ysch_moth, data=subset(scores2plot, sex=="M" & cohort=="BCS"))
% r.ext.m.mcs <- lm(EXT ~ as.factor(incq), data=subset(scores2plot, sex=="M" & cohort=="MCS"))
% r.ext2.m.mcs <- lm(EXT ~ as.factor(incq) + bwt + as.factor(smkpr) + ysch_fath + ysch_moth, data=subset(scores2plot, sex=="M" & cohort=="MCS"))
% r.int.m.bcs <- lm(INT ~ as.factor(incq), data=subset(scores2plot, sex=="M" & cohort=="BCS"))
% r.int2.m.bcs <- lm(INT ~ as.factor(incq) + bwt + as.factor(smkpr) + ysch_fath + ysch_moth, data=subset(scores2plot, sex=="M" & cohort=="BCS"))
% r.int.m.mcs <- lm(INT ~ as.factor(incq), data=subset(scores2plot, sex=="M" & cohort=="MCS"))
% r.int2.m.mcs <- lm(INT ~ as.factor(incq) + bwt + as.factor(smkpr) + ysch_fath + ysch_moth, data=subset(scores2plot, sex=="M" & cohort=="MCS"))
% 
% stargazer(r.ext.m.bcs, r.ext2.m.bcs, r.int.m.bcs, r.int2.m.bcs, r.ext.m.mcs, r.ext2.m.mcs, r.int.m.mcs, r.int2.m.mcs,
%           column.labels=c("BCS","MCS"), column.separate=c(4,4),
%           dep.var.caption = "", multicolumn = FALSE,
%           omit=c("Constant"),
%           covariate.labels=c(
%                              "Income Q2",
...
% \begin{frame}
% \frametitle{Inequality}
% \framesubtitle{Quantifying the gradient -- females}
% \centering
% \resizebox{12cm}{4cm}{%
% <<FA_REGS_REAL_F, dependson='FA_SCORES', results='asis'>>=
% # regress scores on income
% library(dplyr)
% library(stargazer)
% 
% 
% r.ext.f.bcs <- lm(EXT ~ as.factor(incq), data=subset(scores2plot, sex=="F" & cohort=="BCS"))
% r.ext2.f.bcs <- lm(EXT ~ as.factor(incq) + bwt + as.factor(smkpr) + ysch_fath + ysch_moth, data=subset(scores2plot, sex=="F" & cohort=="BCS"))
% r.ext.f.mcs <- lm(EXT ~ as.factor(incq), data=subset(scores2plot, sex=="F" & cohort=="MCS"))
% r.ext2.f.mcs <- lm(EXT ~ as.factor(incq) + bwt + as.factor(smkpr) + ysch_fath + ysch_moth, data=subset(scores2plot, sex=="F" & cohort=="MCS"))
% r.int.f.bcs <- lm(INT ~ as.factor(incq), data=subset(scores2plot, sex=="F" & cohort=="BCS"))
% r.int2.f.bcs <- lm(INT ~ as.factor(incq) + bwt + as.factor(smkpr) + ysch_fath + ysch_moth, data=subset(scores2plot, sex=="F" & cohort=="BCS"))
% r.int.f.mcs <- lm(INT ~ as.factor(incq), data=subset(scores2plot, sex=="F" & cohort=="MCS"))
% r.int2.f.mcs <- lm(INT ~ as.factor(incq) + bwt + as.factor(smkpr) + ysch_fath + ysch_moth, data=subset(scores2plot, sex=="F" & cohort=="MCS"))
% 
% 
% 
% stargazer(r.ext.f.bcs, r.ext2.f.bcs, r.int.f.bcs, r.int2.f.bcs, r.ext.f.mcs, r.ext2.f.mcs, r.int.f.mcs, r.int2.f.mcs,
%           column.labels=c("BCS","MCS"), column.separate=c(4,4),
%           dep.var.caption = "", multicolumn = FALSE,
%           omit=c("Constant"),
%           covariate.labels=c(
...
% \centering
% \resizebox{!}{4cm}{%
% <<FA_RETURNS, dependson='FA_SCORES_SELECT', results='asis'>>=
% # regress scores on income
% library(dplyr)
% library(stargazer)
% 
...
\begin{frame}[noframenumbering]
\frametitle{Inequality \hyperlink{meanci}{\beamerreturnbutton{Back}}}\label{loessext}
\framesubtitle{Visualising the gradient -- inflation- and growth-adjusted income}
\begin{figure}
<<LOESS_REAL_EXT, dependson='LOESS_REAL', dev='tikz', fig.height=8, fig.width=12, out.width='.95\\linewidth'>>=
ploess.real.ext
@
\end{figure}
\end{frame}
...
\begin{frame}[noframenumbering]
\frametitle{Inequality \hyperlink{meanci}{\beamerreturnbutton{Back}}}\label{loessint}
\framesubtitle{Visualising the gradient -- inflation- and growth-adjusted income}
\begin{figure}
<<LOESS_REAL_INT, dependson='LOESS_REAL', dev='tikz', fig.height=8, fig.width=12, out.width='.95\\linewidth'>>=
ploess.real.int
@
\end{figure}
\end{frame}
...
\begin{figure}
<<LOESS_INFL_EXT, dependson='LOESS_INFL', dev='tikz', fig.height=8, fig.width=12, out.width='.95\\linewidth'>>=
ploess.infl.ext
@
\end{figure}
\end{frame}
...
\framesubtitle{Visualising the gradient -- inflation-adjusted income}
\begin{figure}
<<LOESS_INFL_INT, dependson='LOESS_INFL', dev='tikz', fig.height=8, fig.width=12, out.width='.95\\linewidth'>>=
ploess.infl.int
@
\end{figure}
\end{frame}
...
\centering
\resizebox{!}{4cm}{%
<<FA_REGS_INFL, dependson='FA_SCORES_SELECT', results='asis'>>=
# regress scores on income
library(dplyr)
library(stargazer)
