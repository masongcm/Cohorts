...
read_chunk(here("codes/R/7_counterf.R"))
read_chunk(here("codes/R/7_counterf_graphs.R"))
read_chunk(here("codes/R/7_counterf_tabs.R"))
read_chunk(here("codes/R/7_counterf_seqdec.R"))
read_chunk(here("codes/R/8_regs.R"))
...
\centering
\footnotesize
\caption{Item Commonalities -- Males}\label{tab:comm_m}
<<MEANTABLE_RES_M, dependson="MEANTABLE", results='asis'>>=
require(xtable)
...
\centering
\footnotesize
\caption{Item Commonalities -- Females}\label{tab:comm_f}
<<MEANTABLE_RES_F, dependson="MEANTABLE", results='asis'>>=
require(xtable)
...
\clearpage
\begin{figure}
<<RAWHIST, dev='tikz', fig.height=10, fig.width=10, out.width='1\\linewidth'>>=
@
\caption{Frequency of Raw Scores}
{\scriptsize \emph{Notes}: The figure shows frequencies of raw (sum) scores separately by cohort. Raw scores are obtained using the same 11 items in the CFA model, building a categorical index. Children for which each behaviour ``certainly applies/true" are assigned zero points, one point for ``somewhat applies/true" and two points for ``does not apply/not true". For binary items, one point is assigned for ``true". The raw scores are the sum of the points across the 11 behaviours considered, six for EXT and five for INT. \par}
\end{figure}
...
<<INEQ_STATS, dependson="FA_SCORES_MAIN", results="markup">>=
...
cat("Quantile differences in income by cohort")
tb
...
tb.ext$d9010 <- abs(tb.ext$x[,"75%"]-tb.ext$x[,"25%"])
tb.ext$d5010 <- abs(tb.ext$x[,"50%"]-tb.ext$x[,"10%"])
tb.ext$d9050 <- abs(tb.ext$x[,"90%"]-tb.ext$x[,"50%"])
cat("\n Quantile differences in EXT by cohort/gender")
tb.ext
...
tb.int$d9010 <- abs(tb.int$x[,"75%"]-tb.int$x[,"25%"])
tb.int$d5010 <- abs(tb.int$x[,"50%"]-tb.int$x[,"10%"])
tb.int$d9050 <- abs(tb.int$x[,"90%"]-tb.int$x[,"50%"])
cat("\n Quantile differences in INT by cohort/gender")
tb.int
...
{\scriptsize \emph{Notes}: The figure plots the mean and its 95\% confidence interval across harmonised social class of the cohort member's family at 10 years of age. \par}
\end{figure}

\begin{figure}
<<FACINEQ_YS, dependson="FA_SCORES_MAIN", dev='tikz', fig.height=12, fig.width=10, out.width='1\\linewidth'>>=
@
\caption{Mean scores across maternal schooling}\label{fig:meanci_ys}
{\scriptsize \emph{Notes}: The figure plots the mean and its 95\% confidence interval across grouped years of schooling of the cohort member's mother at 5 years of age. \par}
\end{figure}

\begin{figure}
<<FACINEQ_ME, dependson="FA_SCORES_MAIN", dev='tikz', fig.height=12, fig.width=10, out.width='1\\linewidth'>>=
@
\caption{Mean scores across maternal employment}\label{fig:meanci_me}
{\scriptsize \emph{Notes}: The figure plots the mean and its 95\% confidence interval by employment status of the cohort member's mother at 5 years of age. \par}
\end{figure}
...
<<REGRESSIONS, dependson="SELECTVARS">>=
# run all regression models
@

\begin{table}
\caption{Quantifying the gradient -- Males externalising}
\centering
\resizebox{\textwidth}{!}{
<<REGS_EXT_M, results='asis', dependson="REGRESSIONS">>=
library(stargazer)
stargazer(r.ext$BCS.M[[1]], r.ext$BCS.M[[2]], r.ext$BCS.M[[3]], r.ext$MCS.M[[1]], r.ext$MCS.M[[2]], r.ext$MCS.M[[3]],
          column.labels=c("BCS","BCS","BCS","MCS","MCS","MCS"),
...
\begin{table}
\caption{Quantifying the gradient -- Females externalising}
\centering
\resizebox{\textwidth}{!}{
<<REGS_EXT_F, results='asis', dependson="REGRESSIONS">>=
library(stargazer)
stargazer(r.ext$BCS.F[[1]], r.ext$BCS.F[[2]], r.ext$BCS.F[[3]], r.ext$MCS.F[[1]], r.ext$MCS.F[[2]], r.ext$MCS.F[[3]],
          column.labels=c("BCS","BCS","BCS","MCS","MCS","MCS"),
          dep.var.caption = "", multicolumn = FALSE,
          #omit=c("Constant", paste0("region", c(2:9))),
          no.space=TRUE,
...
\begin{table}
\caption{Quantifying the gradient -- Males internalising}
\centering
\resizebox{\textwidth}{!}{
<<REGS_INT_M, results='asis', dependson="REGRESSIONS">>=
library(stargazer)
stargazer(r.int$BCS.M[[1]], r.int$BCS.M[[2]], r.int$BCS.M[[3]], r.int$MCS.M[[1]], r.int$MCS.M[[2]], r.ext$MCS.M[[3]],
          column.labels=c("BCS","BCS","BCS","MCS","MCS","MCS"),
          dep.var.caption = "", multicolumn = FALSE,
          #omit=c("Constant", paste0("region", c(2:9))),
          no.space=TRUE,
...
\begin{table}
\caption{Quantifying the gradient -- Females internalising}
\centering
\resizebox{\textwidth}{!}{
<<REGS_INT_F, results='asis', dependson="REGRESSIONS">>=
library(stargazer)
stargazer(r.int$BCS.F[[1]], r.int$BCS.F[[2]], r.int$BCS.F[[3]], r.int$MCS.F[[1]], r.int$MCS.F[[2]], r.int$MCS.F[[3]],
          column.labels=c("BCS","BCS","BCS","MCS","MCS","MCS"),
          dep.var.caption = "", multicolumn = FALSE,
          #omit=c("Constant", paste0("region", c(2:9))),
          no.space=TRUE,
...
\begin{landscape}
\begin{table}
\caption{Effect of behavioural skills on adolescent outcomes -- BCS males}
\centering
\resizebox{24cm}{!}{
<<REGS_OUTC_BCS_M, results='asis', dependson="REGRESSIONS">>=
library(stargazer)
# get mean dependent vars
dvmeans <- colMeans(bcsoutc[bcsoutc$sex=="F",bcsoutcvars], na.rm = T)
dvmeans <- round(unlist(lapply(dvmeans, function(x) rep(x,2))),3)

stargazer(bcsoutcmod_m_uc[[1]], bcsoutcmod_m_c[[1]],
          bcsoutcmod_m_uc[[2]], bcsoutcmod_m_c[[2]],
          bcsoutcmod_m_uc[[3]], bcsoutcmod_m_c[[3]],
          bcsoutcmod_m_uc[[4]], bcsoutcmod_m_c[[4]],
          bcsoutcmod_m_uc[[5]], bcsoutcmod_m_c[[5]],
          bcsoutcmod_m_uc[[6]], bcsoutcmod_m_c[[6]],
          add.lines = list(c("Mean Dep Var", dvmeans)),
          no.space=TRUE,
...
\begin{landscape}
\begin{table}
\caption{Effect of behavioural skills on adolescent outcomes -- MCS males}
\centering
\resizebox{24cm}{!}{
<<REGS_OUTC_MCS_M, results='asis', dependson="REGRESSIONS">>=
library(stargazer)
# get mean dependent vars
dvmeans <- colMeans(mcsoutc[mcsoutc$sex=="F",mcsoutcvars], na.rm = T)
dvmeans <- round(unlist(lapply(dvmeans, function(x) rep(x,2))),3)

stargazer(mcsoutcmod_m_uc[[1]], mcsoutcmod_m_c[[1]],
          mcsoutcmod_m_uc[[2]], mcsoutcmod_m_c[[2]],
          mcsoutcmod_m_uc[[3]], mcsoutcmod_m_c[[3]],
          mcsoutcmod_m_uc[[4]], mcsoutcmod_m_c[[4]],
          mcsoutcmod_m_uc[[5]], mcsoutcmod_m_c[[5]],
          add.lines = list(c("Mean Dep Var", dvmeans)),
          no.space=TRUE,
          omit.stat= c("rsq", "f", "ser", "aic", "ll"),
          single.row=T,
...
\begin{landscape}
\begin{table}
\caption{Effect of behavioural skills on adolescent outcomes -- BCS females}
\centering
\resizebox{24cm}{!}{
<<REGS_OUTC_BCS_F, results='asis', dependson="REGRESSIONS">>=
library(stargazer)
# get mean dependent vars
dvmeans <- colMeans(bcsoutc[bcsoutc$sex=="F",bcsoutcvars], na.rm = T)
dvmeans <- round(unlist(lapply(dvmeans, function(x) rep(x,2))),3)

stargazer(bcsoutcmod_f_uc[[1]], bcsoutcmod_f_c[[1]],
          bcsoutcmod_f_uc[[2]], bcsoutcmod_f_c[[2]],
          bcsoutcmod_f_uc[[3]], bcsoutcmod_f_c[[3]],
          bcsoutcmod_f_uc[[4]], bcsoutcmod_f_c[[4]],
          bcsoutcmod_f_uc[[5]], bcsoutcmod_f_c[[5]],
          bcsoutcmod_f_uc[[6]], bcsoutcmod_f_c[[6]],
          add.lines = list(c("Mean Dep Var", dvmeans)),
          no.space=TRUE,
          omit.stat= c("rsq", "f", "ser", "aic", "ll"),
          single.row=T,
...
\begin{landscape}
\begin{table}
\caption{Effect of behavioural skills on adolescent outcomes -- MCS females}
\centering
\resizebox{24cm}{!}{
<<REGS_OUTC_MCS_F, results='asis', dependson="REGRESSIONS">>=
library(stargazer)
# get mean dependent vars
dvmeans <- colMeans(mcsoutc[mcsoutc$sex=="F",mcsoutcvars], na.rm = T)
dvmeans <- round(unlist(lapply(dvmeans, function(x) rep(x,2))),3)

stargazer(mcsoutcmod_f_uc[[1]], mcsoutcmod_f_c[[1]],
          mcsoutcmod_f_uc[[2]], mcsoutcmod_f_c[[2]],
          mcsoutcmod_f_uc[[3]], mcsoutcmod_f_c[[3]],
          mcsoutcmod_f_uc[[4]], mcsoutcmod_f_c[[4]],
          mcsoutcmod_f_uc[[5]], mcsoutcmod_f_c[[5]],
          add.lines = list(c("Mean Dep Var", dvmeans)),
          no.space=TRUE,
          omit.stat= c("rsq", "f", "ser", "aic", "ll"),
          single.row=T,
...
<<COUNTERF_SEQDEC_ESTIM, dependson="SELECTVARS">>=
# estimate sequential decompositions
@
...
\begin{landscape}
\begin{figure}
<<COUNTERF_SEQDEC_EXT, dependson='COUNTERF_SEQDEC_PLOT', dev='tikz', fig.height=10, fig.width=16, out.width='1\\linewidth'>>=
cfp.seqdec.ext
@
\caption{Counterfactual sequential decomposition -- externalising skills}\label{fig:counterf_seq_ext}
{\scriptsize \emph{Notes}: \par}
\end{figure}
\end{landscape}
...
\begin{landscape}
\begin{figure}
<<COUNTERF_SEQDEC_INT, dependson='COUNTERF_SEQDEC_PLOT', dev='tikz', fig.height=10, fig.width=16, out.width='1\\linewidth'>>=
cfp.seqdec.int
@
\caption{Counterfactual sequential decomposition -- internalising skills}\label{fig:counterf_seq_int}
{\scriptsize \emph{Notes}: \par}
\end{figure}
\end{landscape}
...
<<BCSEFARES, dependson='BCSEFA', results='markup'>>=
@

<<MCSEFARES, dependson='BCSEFA', results='markup'>>=
@

\begin{table}[ht]
\caption{Number of factors} \label{tab:nfac}
...
\begin{table}[ht]
\caption{Summary statistics} \label{tab:lvpars}
\centering
<<SUMSTATS, results='asis'>>=

datacompl <- scores2plot[complete.cases(scores2plot[,decvars]),] # complete cases
# expand dummies
datasum <- cbind(
  datacompl[,c("EXT","INT","cohortsex")], 
  dummy.data.frame(datacompl[,c(decvars)], sep = "")
)

# statistics by group
sumtab <- data.frame(
  aggregate(datasum, by=list(datasum$cohortsex), FUN = function(x) c(mn = mean(x, na.rm=T)))
)

# drop unused factors and variables
sumtab <- sumtab[, substring(names(sumtab), nchar(names(sumtab))) != "0"]
sumtab <- sumtab[, ! names(sumtab) %in% c("EXT","INT","cohortsex","cohortBCS","cohortMCS")]
group <- sumtab$Group.1
sumtabt <- as.data.frame(t(sumtab[,-1]))
colnames(sumtabt) <- group
# rownames to var
sumtabt$var <- rownames(sumtabt)
# reorder columns
sumtabt <- sumtabt[c("var", "BCS.M", "MCS.M", "BCS.F", "MCS.F")]

# table
print.xtable(xtable(sumtabt,
                    align="L{0cm}L{3cm}C{1.2cm}C{2cm}C{2cm}C{2cm}",
                    digits = 3
                    ),
             size="\\small",
             include.rownames=F,
