...
read_chunk(here("codes/R/6_decvars.R"))
...
% 
% \begin{table}
% \caption{Quantifying the gradient}
% \centering
% \resizebox{\textwidth}{!}{
% <<FA_REGS_REAL_M, results='asis'>>=
% # regress scores on income
% library(stargazer)
% 
% decvars_reg <- c(decvars, "cohort") # add cohort
% interact_reg <- paste0(decvars_reg[! decvars_reg %in% c("cohort")], "*cohort")
% 
% form_ext_reg <- as.formula(paste("EXT ~ ", paste0(decvars2, collapse = '+')))
% form.ext2i <- as.formula(paste("EXT ~ ", paste0(c(decvars2, interact2), collapse = '+')))
% form.int2 <- as.formula(paste("INT ~ ", paste0(decvars2, collapse = '+')))
% form.int2i <- as.formula(paste("INT ~ ", paste0(c(decvars2, interact2), collapse = '+')))
% 
% r.ext.m  <- lm(form.ext2,  data=subset(scores2plot, sex=="M"))
% r.exti.m <- lm(form.ext2i, data=subset(scores2plot, sex=="M"))
% r.ext.f  <- lm(form.ext2,  data=subset(scores2plot, sex=="F"))
% r.exti.f <- lm(form.ext2i, data=subset(scores2plot, sex=="F"))
% r.int.m  <- lm(form.int2,  data=subset(scores2plot, sex=="M"))
% r.inti.m <- lm(form.int2i, data=subset(scores2plot, sex=="M"))
% r.int.f  <- lm(form.int2,  data=subset(scores2plot, sex=="F"))
% r.inti.f <- lm(form.int2i, data=subset(scores2plot, sex=="F"))
% 
% stargazer(r.ext.m, r.exti.m, r.ext.f, r.exti.f, r.int.m, r.inti.m, r.int.f, r.inti.f,
%           column.labels=c("M","M","F","F","M","M","F","F"), #,"BCS F","MCS F","BCS F","MCS F"),
...
\begin{landscape}
\begin{figure}
<<COUNTERF_SEQDEC_EXT, dependson='COUNTERF_SEQDEC_PREP', dev='tikz', fig.height=10, fig.width=16, out.width='1\\linewidth'>>=
cfp.seqdec.ext
@
...
\begin{landscape}
\begin{figure}
<<COUNTERF_SEQDEC_INT, dependson='COUNTERF_SEQDEC_PREP', dev='tikz', fig.height=10, fig.width=16, out.width='1\\linewidth'>>=
cfp.seqdec.int
@
