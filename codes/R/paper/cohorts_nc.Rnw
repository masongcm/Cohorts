...
read_chunk(here("codes/R/6_ineq.R"))
...
<<INCINEQ_STATS, dependson="FA_SCORES_MAIN", results="markup">>=
tb <- data.frame(
  aggregate(scores2plot$faminc_infl, by=list(scores2plot$cohort), FUN = function(x) quantile(x, c(.1, .25, .75, .9), na.rm=T))
)
