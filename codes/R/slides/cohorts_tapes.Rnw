...
\item \textbf{Q1}: Can we derive \textbf{comparable measures} of socioemotional skills across populations?
    \bi
    \item Does the same instrument measure the same constructs in the same way:
      \bi 
...
\item \textbf{Measurement Invariance (MI)} analysis
  ...
  \item Set up categorical Item Factor Analysis (IFA) model
  \item Pool the two cohorts together, and estimate models of different invariance level -- following \cite{Wu2016}
...
\begin{frame}
\frametitle{MI}\label{misteps}
\framesubtitle{Invariance steps \hyperlink{miapp1}{\beamerbutton{Details}}}
\bnm
...
print(xtable(table,
                    align="L{0cm}L{.3\\textwidth}C{.1\\textwidth}C{.15\\textwidth}",
                    digits = 4
                    ),
             size="\\footnotesize",
             include.rownames=F,
            ...
            "Model & Num. params & RMSEA \\\\"
             ...
             sanitize.text.function = function(x){x},
             sanitize.rownames.function = function(x){""}, # to not print row numbers
             tabular.environment = "tabular", width = "5\\textwidth",
             floating = FALSE
)
