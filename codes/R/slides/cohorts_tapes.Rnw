...
\bi
  ...
  \item Usually, socioemotional constructs are measured using validated scales
    ...
    \item Obtained by aggregating different items
    \ei
  \item Can we use these scales to compare different cohorts, by gender?
...
\item \textbf{Key findings}
  ...
  \item Increased importance of pregnancy factors, decreased importance of paternal education, in explaining skill inequality by maternal education
...
\item Not always \textbf{consistent}
  ...
  \item Harmonisation effort underway (CLOSER)
  \ei
\ei
...
\item $\Rightarrow$ New 11-item subscale
\ei
...
\bi
\item Factor analysis routinely used to construct measures of socioemotional skills \citep{Conti2014,Attanasio2017c}
\item Starts with \emph{exploratory} step
\item Decompose the covariance matrix of items to assess:
  \bi \item the dimensionality of the underlying factor structure (how many latent factors)
  \item which items measure which latent factor \ei
...
\item This can be tested in our data
\ei
...
\item Dimensionality
  \bi
  \item Methods for assessing dimensionality imply 1-3 factors \hyperlink{numfac}{\beamerbutton{Det.}}
  \item We choose \textbf{two factors} to maximise interpretability
  \ei
\item Structure
  \bi
  \item Exploratory FA using weighted least squares and \textit{oblimin} rotation
  \item Factors can be coherently interpreted as \textbf{externalising} ($\sim$ anti-social) and \textbf{internalising} ($\sim$ neurotic)
  \ei
\ei 
\pause
\vspace{6pt}
...
\textbf{Measurement Invariance (MI)} analysis
  ...
  \item Procedure:
    \bnm
    \item Start from the most general model -- parameters allowed to differ across groups
...
\bi
\item Observed categorical items $X_1$-$X_{11}$ measuring latent factor $\theta$
\item Latent propensity to endorse each item $i$ for child $j$ in group $c$ \citep{Muthen1984}
...
\vspace{.2cm}
\item We follow the approach in \cite{Wu2016a}
  \bi
...
\bi
\item We use RMSE to compare fit between models with different invariance restriction
\item Restricting only thresholds and loadings provides a good fit \hyperlink{fit}{\beamerbutton{Fit details}}
...
\bi
\item We can use the model with non-invariant intercepts to score the latent factors (externalising and internalising dimensions)
\item Estimated parameters: \hyperlink{measpars}{\beamerbutton{Measurement}}\hyperlink{lvpars}{\beamerbutton{Latent variable}}
\item Use Empirical Bayes approach to predict individual scores \citep{Wu2013}
\item All scores are coded so that \emph{\textbf{a higher value means better socioemotional skills}}
\item \textbf{CAUTION}: location differences by cohort/gender are not interpretable!
\ei
\end{frame}
...
\item Is this an improvement on using simple sum scores?
  ...
  \item Every 11-dimensional pattern of response in the data can be assigned a different score
...
\framesubtitle{Mother compulsory education \hyperlink{ineq_ys}{\beamerbutton{Age left education}}}
\vspace{-.2cm}
\begin{figure}
...
\bi
\item Unconditionally, the distribution is more spread out (except for girls' internalising)
...
\item \cite{McCulloch2000,Blanden2008,Fitzsimons2017}
\item Linear regression (Tobit results are very similar)
\item Merge information from the birth and 5 year surveys \hyperlink{detvars}{\beamerbutton{Variables}}
\ei
\end{frame}
...
\framesubtitle{Gelbach (2016)}

\bi
\item We decompose the difference between children of mothers with compulsory education vs. post-compulsory
\item Use the same groups of covariates as above
\item Approach by \cite{Gelbach2016}
  \bi
...
\begin{frame}
\frametitle{Decomposition (EXT)}\label{gelb_ext_f}
\framesubtitle{Maternal compulsory schooling \hyperlink{gelb_ext}{\beamerbutton{Table}}}
\begin{figure}
...
\begin{frame}
\frametitle{Decomposition (INT)}\label{gelb_int_f}
\framesubtitle{Maternal compulsory schooling \hyperlink{gelb_int}{\beamerbutton{Table}}}
\begin{figure}
...
\bi
\item We examine how sociemotional skills at age 5 map into outcomes later in the life course
  \bi 
  \item BCS: Age 16, 30, 38, 42
  \item MCS: Age 14 (last available survey)
  \ei
\item Previous work has mostly focused on single cohorts\cite{Carneiro2007,Conti2010a,Joshi2014}
\item Focus on behaviours and labour market outcomes
\pause
\item Include a \textbf{cognitive factor} at age 5, using available tests
  ...
  \item MCS: British Ability Scales (Naming Vocabulary, Picture Similarities, Pattern Construction)
...
\bi
\item Using a novel psychometric method, we derived two measures of non-cognitive skills at age five that is comparable across two UK cohorts
...
\item The determinants of socioemotional skills have changed
  ...
  \ei
\item For both cohorts, socioemotional skills predict adolescent outcomes, for health outcomes over and above cognitive skills
\ei
...
\framesubtitle{Variables \hyperlink{detintro}{\beamerreturnbutton{Back}}}
