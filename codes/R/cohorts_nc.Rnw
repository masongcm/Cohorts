...
read_chunk("/Users/giacomomason/Documents/Projects/CohortStudies/codes/R/3_mifa_cog.R")
read_chunk("/Users/giacomomason/Documents/Projects/CohortStudies/codes/R/4_graphs.R")
...
For BCS, I augment the 20-item Rutter Scale with 4 other parent-reported questions from the parental questionnaire. 
\begin{enumerate}
...
<<MEANTABLE_RES, dependson="LOAD_DATA", results='asis'>>=
require(xtable)
...
\begin{split}
X^*_{ijc} &=\nu_{ci} + \lambda_{ci}\theta^{EXT}_{cj} + u_{cji} \qquad \text{for} \; i=1,\dots, 6 \\
X^*_{ijc} &=\nu_{ci} + \lambda_{ci}\theta^{INT}_{cj} + u_{cji} \qquad \text{for} \; i=7,\dots, 11
\end{split}
\end{align}
...
$$X_{cji} = s \qquad \text{if} \; \tau_{s,ci} \leq X^*_{cji} < \tau_{s+1,ci} \qquad \text{for} \; s=0,1,2$$
with $\tau_{0,ci}=-\infty$ and $\tau_{3,ci}=+\infty$.
...
\bnm[label=\Alph*:]
\item Delta parameterisation {[WE{\textDelta}]} \citep{Wu2016}

For all groups: $$\text{diag}(\bm{\Phi}) = \bm{I}, \quad \bm{\kappa}=\bm{0}, \quad \bm{\nu}=\bm{0}, \quad \text{and} \; \text{diag}(\bm{\Sigma})=\bm{I}.$$

\item Theta parameterisation {[WE{\textTheta}]} \citep{Wu2016}
...
\centering
\small
\caption{Fit indices}\label{tab:fit}
...
print(xtable(table,
                    align="L{0cm}L{.18\\textwidth}C{.05\\textwidth}C{.07\\textwidth}C{.07\\textwidth}C{.06\\textwidth}C{.06\\textwidth}C{.06\\textwidth}C{.06\\textwidth}C{.06\\textwidth}C{.06\\textwidth}",
                    digits = 4
                    ),
             size="\\small",
             include.rownames=F,
             ...
             add.to.row = list(pos = c(list(0)),
                              ...
                              # header
                                    paste("
                                            Model & Num. params & $\\chi^2$ & RMSEA & MFI & CFI & G-hat & \\textDelta MFI & \\textDelta CFI & \\textDelta G-hat \\\\
                                          "
                                            , sep="")
                                    )),
             sanitize.text.function = function(x){x},
...
\resizebox{\textwidth}{!}{
<<RES_FA_PAR, dependson="FA_FINAL_PARS", results='asis'>>=
require(xtable)
...
print.xtable(xtable(table,
                    # align = "ll|cccc|cccc"
                    align="L{0cm}L{2cm}C{2.5cm}C{2.5cm}C{2.5cm}C{2.5cm}C{1.5cm}",
                    digits = 3
                    ),
             size="\\small",
             include.rownames=F,
...
\centering
<<RES_FA_LV, dependson="FA_FINAL_PARS", results='asis'>>=
require(xtable)
...
print.xtable(xtable(table,
                    align = "llccc",
                    # align="L{0cm}L{.1\\textwidth}L{.2\\textwidth}L{.15\\textwidth}L{.15\\textwidth}L{.15\\textwidth}L{.15\\textwidth}",
                    digits = 3
                    ),
             size="\\small",
             include.rownames=F,
             include.colnames=F,
             booktabs = T,
             add.to.row = list(pos = c(list(0,2)),
                              command = c(
                              # header
                                    paste(" & \\multicolumn{3}{c}{\\textbf{BCS}} \\\\ \\midrule
                                            & \\multicolumn{1}{c}{\\textbf{Mean}}
                                            & \\multicolumn{2}{c}{\\textbf{Covariance}} \\\\
                                            && $\\theta^{EXT}$ & $\\theta^{INT}$ \\\\", sep=""),
                                    paste(" \\midrule & \\multicolumn{3}{c}{\\textbf{MCS}} \\\\ \\midrule
                                            & \\multicolumn{1}{c}{\\textbf{Mean}}
                                            & \\multicolumn{2}{c}{\\textbf{Covariance}} \\\\
                                            && $\\theta^{EXT}$ & $\\theta^{INT}$ \\\\", sep="")
                                    )),
             sanitize.text.function = function(x){x},
             sanitize.rownames.function = function(x){""}, # to not print row numbers
             tabular.environment = "tabularx", width = ".5\\textwidth",
             floating = FALSE
)
...
\begin{table}[ht]
\caption{Estimated latent variable means and covariances -- model with restricted intercepts} \label{tab:lvpars2}
\centering
...
Although the results of our measurement invariance analysis imply that comparison of latent means is not justified, the model with restricted loadings and thresholds can nevertheless be used to investigate how inequality of noncognitive skills has changed between the two cohorts. The variances of the $EXT$ and $INT$ factors are estimated to be larger in the MCS children by \Sexpr{round(lv.covs$MCS[1,1]-1,2)} and \Sexpr{round(lv.covs$MCS[2,2]-1,2)} of a standard deviation, respectively, indicating larger dispersion.
...
reg.bcs.ext <- lm(EXT~as.factor(incq), data=subset(facandraw, cohort=="BCS"))
bcs.51.ext <- round(reg.bcs.ext$coefficients["as.factor(incq)5"],2)
reg.bcs.int <- lm(INT~as.factor(incq), data=subset(facandraw, cohort=="BCS"))
bcs.51.int <- round(reg.bcs.int$coefficients["as.factor(incq)5"],2)

reg.mcs.ext <- lm(EXT~as.factor(incq), data=subset(facandraw, cohort=="MCS"))
mcs.51.ext <- round(reg.mcs.ext$coefficients["as.factor(incq)5"],2)
reg.mcs.int <- lm(INT~as.factor(incq), data=subset(facandraw, cohort=="MCS"))
mcs.51.int <- round(reg.mcs.int$coefficients["as.factor(incq)5"],2)

@

We can derive scores for $\theta^{EXT}$ and $\theta^{INT}$ directly, and plot them against socioeconomic measures observed in the data. Our main socioeconomic indicator is a measure of weekly total family net income in the prices of the interview year.\footnote{This measure is harmonised across cohorts and derived as part of CLOSER, which is funded by the ESRC.} Using the estimated parameters for the model with invariant thresholds and loadings (but non-invariant intercepts), I compute the scores for each individual in the sample (using the Empirical Bayes modal approach). In \autoref{fig:boxscores}, we can see that the gradient in noncognitive skills along family income quintiles is more pronounced in the MCS cohort. In particular, a five-year old at the top quintile of the income distribution in 1975 has a better score by \Sexpr{bcs.51.ext} in the externalising dimension and \Sexpr{bcs.51.int} in the internalising dimension, compared to a child at the bottom quintile. In 2005, the differences amount to \Sexpr{mcs.51.ext} and \Sexpr{mcs.51.int}, respectively.


\clearpage
\begin{figure}
<<FACDENS, dev='tikz', fig.height=5, fig.width=10, out.width='1\\linewidth'>>=
@
\caption{Density of Scored Factors}
{\scriptsize \emph{Notes}: The figure shows estimated kernel densities of the scored factors from model \eqref{eq:finalmod}, separately by cohort.  \par}
\end{figure}
...
{\scriptsize \emph{Notes}: The figure plots the scored factors from model \eqref{eq:finalmod} on the vertical axis against the ``raw" scores on the horizontal axis, i.e. scores obtained by simply summing the number of positive answers to each of the items. \par}
\end{figure}

\begin{figure}
<<FACSCAT, dev='tikz', fig.height=5, fig.width=10, out.width='1\\linewidth'>>=
@
\caption{Scatterplots of factor scores by cohort}
{\scriptsize \emph{Notes}: The figure plots the scored factors from model \eqref{eq:finalmod} separately by cohort. The externalising and internalising scores are plotted against one another. \par}
\end{figure}

\begin{figure}
<<FACINEQ, dev='tikz', fig.height=10, fig.width=10, out.width='1\\linewidth'>>=
@
\caption{Distribution of scores across income quintiles}\label{fig:boxscores}
{\scriptsize \emph{Notes}: The figure plots the distribution of the scored factors from model \eqref{eq:finalmod} across the quintiles in the income distribution. \par}
\end{figure}

\begin{figure}
<<FACLOESS, dev='tikz', fig.height=5, fig.width=10, out.width='1\\linewidth'>>=
@
\caption{Scores and family income}
{\scriptsize \emph{Notes}: The figure plots LOESS of the scored factors from model \eqref{eq:finalmod} against family income. \par}
\end{figure}
