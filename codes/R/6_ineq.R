...
decvars <- c("region","faminc_infl","scl10b","ysch_moth5","smkpr","mothageb","numch5")
...
for (g in c("M","F")) {
  for (c in c("BCS", "MCS")) {
    ...
    subs$EXTres <- residuals(lm(form.ext, data=subs, na.action=na.exclude))
    subs$INTres <- residuals(lm(form.int, data=subs, na.action=na.exclude))
    df.final <- rbind(df.final,subs)
  }
}
# KEEP ONLY OBS WITH COMPLETE DATA
df.final <- df.final[complete.cases(df.final[,c('EXT', 'INT', decvars)]),]
