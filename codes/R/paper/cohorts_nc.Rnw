...
{\scriptsize \emph{Notes}: The figure shows frequencies of raw (sum) scores separately by cohort.  \par}
\end{figure}
...
datacompl <- scores2plot[complete.cases(scores2plot[,decvars2]),] # complete cases
# expand dummies
datasum <- cbind(
  datacompl[,c("EXT","INT","cohortsex")], 
  dummy.data.frame(datacompl[,c(decvars2)], sep = "")
)
